...
pub fn init(_flags) -> #(AppModel, Effect(AppMsg)) {
  ...
  let load_sims_effect = effect.map(army_update.load_saved_simulations(), ArmyMsg)

  // Загружаем сохранённые профили для army_simulator
  let load_profiles_effect = effect.map(army_update.load_saved_profiles(), ArmyMsg)

  #(model, effect.batch([init_worker_effect, load_sims_effect, load_profiles_effect]))
}
